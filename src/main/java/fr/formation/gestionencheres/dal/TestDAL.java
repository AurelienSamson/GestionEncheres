...
//				LocalDate.now(), 100, 150, "EC", aurelien, info);
//		ArticleEnVente souris = new ArticleEnVente("Aerox 3", "Souris Steelseries", LocalDate.now(), LocalDate.now(),
//				70, 110, "EC", guillaume, info);
//		ArticleEnVente peinture = new ArticleEnVente("Le cris Edward Munch", "Peinture impressioniste", LocalDate.now(),
//				LocalDate.now(), 500, 1350, "VE", delphine, art);
//		ArticleEnVente setNapoleon = new ArticleEnVente("set complet vaisselle", "napoléon", LocalDate.now(),
//				LocalDate.now(), 700, 2800, "AN", guillaume, vaisselle);
//		ArticleEnVente mustang = new ArticleEnVente("Mustang GT", "Ford", LocalDate.now(), LocalDate.now(), 36000,
//				53000, "NA", aurelien, voiture);
//
//		Enchere enchereClavier = new Enchere(LocalDateTime.now(), 150, aurelien, clavier);
//		Enchere enchereSouris = new Enchere(LocalDateTime.now(), 110, guillaume, souris);
//		Enchere encherepeinture = new Enchere(LocalDateTime.now(), 1350, delphine, peinture);
//		Enchere enchereSetNapoleon = new Enchere(LocalDateTime.now(), 2800, guillaume, setNapoleon);
//		Enchere enchereMustang = new Enchere(LocalDateTime.now(), 53000, aurelien, mustang);
//
...
//		manager.insertRetraits(mondeville);
		...
		//System.out.println(manager.getArticlesEnVenteByEtatVente("EC"));
